...
namespace Utils {
    public class BackgroundCodeExecutor : MonoBehaviour {
        ...
        /// When you want to manually cancel the execution (for example, when the world has changed and the execution needs to start over), call cancelTokenSource.Cancel() on the returned CancellationTokenSource.
        /// </summary>
        ...
        /// <returns> The CancellationTokenSource that can be used to cancel the background action. </returns>
        public static CancellationTokenSource ExecuteInBackground(Action<CancellationToken> actionToExecute, Action onComplete, Action<Exception>? onError = null, Action? onCancel = null) {
            var executor = Instance;
            var cancelTokenSource = new CancellationTokenSource();
            var cancelToken = cancelTokenSource.Token;
            var task = Task.Run(() => actionToExecute(cancelToken), cancelToken);
            // var task = executor.ExecuteInBackgroundTask(actionToExecute);
        ...
        private void Update() {
            lock (finishedTasksLock) {
                foreach (var (task, onComplete, onError, onCancel, cancelToken) in finishedTasks) {
                    if (cancelToken.IsCancellationRequested && task.Status != TaskStatus.Canceled) {
                        Debug.LogWarning("Task was not canceled, but should have been");
                    }
                    switch (task.Status) {
