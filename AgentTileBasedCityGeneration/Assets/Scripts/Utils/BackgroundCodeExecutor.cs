...
namespace Utils {
    public class BackgroundCodeExecutor : MonoBehaviour {
        ...
        ///     Make sure that actionToExecute calls cancelToken.ThrowIfCancellationRequested() from time to time (cancelling the
        ///     execution).
        ///     When you want to manually cancel the execution (for example, when the world has changed and the execution needs to
        ///     start over), call Cancel() on the returned <see cref="BackgroundTask" />.
        /// </summary>
        /// <param name="actionToExecute"> The action to execute in the background. </param>
        /// <param name="onComplete">
        ///     The action to execute when the background action is completed. Will be executed on the main
        ///     thread (in an Update function)
        /// </param>
        /// <param name="onError">
        ///     The optional action to execute when an error occurs during the background action. Will be
        ///     executed on the main thread (in an Update function)
        /// </param>
        /// <param name="onCancel">
        ///     The optional action to execute when the background action is canceled via the
        ///     <see cref="CancellationTokenSource" /> and the actionToExecute has thrown via ThrowIfCancellationRequested(). Will
        ///     be executed on the main thread (in an Update function)
        /// </param>
