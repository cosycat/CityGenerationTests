//https://github.com/hightower70/QuadtreePoint
...
namespace Utils.DataStructures {
    ...
    public class QuadTreeFloatPointRegion {
        ...
        public bool IsOverlapping(QuadTreeFloatPointRegion in_region) {
            return Math.Abs(CenterX - in_region.CenterX) <= in_region.HalfSize + HalfSize &&
                   Math.Abs(CenterY - in_region.CenterY) <= in_region.HalfSize + HalfSize;
        }
    ...
    public class QuadTreeFloatPoint<T> : IEnumerable<T> where T : IQuadTreeData {
        ...
        public QuadTreeFloatPoint(float in_center_x, float in_center_y, float in_half_size) : this(
        ...
        public QuadTreeFloatPoint(float in_center_x, float in_center_y, float in_half_size, int in_bucket_size) : this(
            new QuadTreeFloatPointRegion(in_center_x, in_center_y, in_half_size), in_bucket_size) { }
        ...
        public void Insert(T in_data) {
            var node = new QuadTreeLeaf(in_data);
        ...
        private void Insert(QuadTreeNode in_current_node, QuadTreeLeaf in_node_to_insert) {
            ...
            var bounds = in_current_node.Bounds;
            var half = bounds.HalfSize / 2;
            var subdivision = new QuadTreeNode[4] {
                new(new QuadTreeFloatPointRegion(bounds.CenterX - half, bounds.CenterY - half, half)),
                new(new QuadTreeFloatPointRegion(bounds.CenterX + half, bounds.CenterY - half, half)),
                new(new QuadTreeFloatPointRegion(bounds.CenterX - half, bounds.CenterY + half, half)),
                new(new QuadTreeFloatPointRegion(bounds.CenterX + half, bounds.CenterY + half, half))
            };
        ...
        public void TraverseNodesAndLeafs(DataProcessCallback in_data_process_callback,
            NodeProcessCallback in_node_process_callback) {
            var stack = new Stack<QuadTreeNode>();
            var current = m_root;
        ...
        public IEnumerable<T> Query(float in_left, float in_top, float in_width, float in_height) {
            ...
            while (current != null) {
                if (current.Children != null) {
                    ...
                    if (children[2].Bounds != null &&
                        children[2].Bounds.IsOverlapping(in_left, in_top, in_width, in_height))
                        stack.Push(children[2]);

                    if (children[1].Bounds != null &&
                        children[1].Bounds.IsOverlapping(in_left, in_top, in_width, in_height))
                        stack.Push(children[1]);

                    if (children[3].Bounds != null &&
                        children[3].Bounds.IsOverlapping(in_left, in_top, in_width, in_height))
                        stack.Push(children[3]);

                    if (children[0].Bounds != null &&
                        children[0].Bounds.IsOverlapping(in_left, in_top, in_width, in_height))
                    ...
                    while (node != null) {
                        if (node.Data.X > in_left && node.Data.X < in_left + in_width && node.Data.Y > in_top &&
                            node.Data.Y < in_top + in_height)
