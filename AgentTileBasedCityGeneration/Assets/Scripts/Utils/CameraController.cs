...
namespace Utils {
    public class CameraController : MonoBehaviour {
        ...
        [field: Tooltip("If set to < MinZoom, the max zoom will be just the size of the world."),
                SerializeField] private float MaxZoom { get; set; } = -1f;
        ...
        private void Update() {
            ...
            if (MaxZoom < 0) {
                MaxZoom = CalculateMaxZoom(Camera.aspect, world.Width, world.Height);
            }
            ...
            var maxY = AllowMovementOutOfBounds ? world.Height : (world.Height - minY < minY) ? minY : world.Height - minY;
