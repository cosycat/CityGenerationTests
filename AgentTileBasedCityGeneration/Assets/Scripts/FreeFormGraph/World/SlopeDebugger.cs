...
namespace FreeFormGraph.World {
    public class SlopeDebugger : MonoBehaviour {
        ...
        private List<(Vector2 a, Vector2 b)> segments = new();
        ...
        private void OnDrawGizmos() {
            if (world != null) {
                ...
                UnityEditor.Handles.Label(highestPoint, $"Highest point: {maxHeight * Constants.METERS_PER_UNIT}",
                    new GUIStyle() { fontSize = 30 });
                UnityEditor.Handles.Label(lowestPoint, $"Lowest point: {minHeight * Constants.METERS_PER_UNIT}",
                    new GUIStyle() { fontSize = 30 });
            }
        }
    }
