...
namespace FreeFormGraph {
    public abstract class StreetGraphGameObject : MonoBehaviour, IStreetGraph {
        ...
        public virtual bool TryFindClosestNode(IEnumerable<IStreetNode> nodes, Vector3 position,
            out IStreetNode foundNode, float threshold = float.MaxValue) {
        ...
        public virtual bool TryFindClosestNode(Vector3 position, out IStreetNode foundNode,
            float threshold = float.MaxValue) {
            return TryFindClosestNode(Nodes, position, out foundNode, threshold);
        }

        public virtual bool TryFindClosestEdge(IEnumerable<IStreetEdge> edges, Vector3 position,
            out IStreetEdge foundEdge, out Vector3 positionOnEdge,
            float threshold = float.MaxValue) {
        ...
        public virtual bool GetOrCreateNode(Vector3 position, float threshold, out IStreetNode node,
            ...
            if (((IStreetGraph)this).TryFindClosestEdge(position, out var edge, out var positionOnEdge,
                    SnapToExistingEdgeThreshold)) {
                Debug.Log(
                    $"IStreetGraph::GetOrCreateNode - Found edge to snap to. Position on edge: {positionOnEdge}, edge: {edge}");
            ...
            var success =
                CreateUnconnectedNode(position,
                    out node); // TODO for optimization: CreateUnconnectedNode will most likely call FindClosestNode again
            isNewlyCreatedNode = success;
            return success;
        }

        public abstract void InsertNodeOnEdge(IStreetEdge edge, Vector3 positionOnEdge, out IStreetNode node,
        ...
        public IStreetNode[] FindAllNodesWithinRange(Vector2 position, float radius) {
            ...
            foreach (var streetNode in Nodes) {
                if (Vector2.Distance(streetNode.Position, position) <= radius) nodes.Add(streetNode);
            }
        ...
        public virtual IStreetEdge[] FindAllEdgesWithinRange(Vector2 position, float radius) {
            ...
            foreach (var streetEdge in Edges) {
                if (streetEdge.GetDistanceEdgeToPosition(position, out _) <= radius) edges.Add(streetEdge);
            }
