...
namespace Graph.SplineBased {
    public static class SplineIntersections {
        ...
        ///     after building a new StreetSegment (which builds a single BezierCurve), this new bezier curve will be checked
        ///     against
        ///     all existing curves for intersections.
        ///     The algorithm used is iterative, which is why there are two parameters which have to be adjusted. The default
        ///     values
        ...
        /// <param name="minDistanceForHit">
        ///     Hit is registered when this distance is met. Bigger value will trigger intersections
        ///     earlier TOOD explain better
        /// </param>
        public static List<CurveIntersection> GetIntersectionsForCurve(
            ...
            for (var step = 0.0f; step < 1; step += stepSize) {
                ...
                foreach (var targetSpline in allSplines) {
                    for (var i = 0; i < targetSpline.GetCurveCount(); i++) {
