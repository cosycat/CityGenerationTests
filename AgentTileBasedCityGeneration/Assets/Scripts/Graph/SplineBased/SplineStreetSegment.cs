    ...
    public class SplineStreetSegment : IStreetEdge {
        ...
        public float GetDistanceEdgeToPosition(Vector3 position, out Vector3 positionOnEdge) {
            ...
            var nearestPointOnLine =
                SplineMath.PointLineNearestPoint(position, closestPoint, secondClosestPoint, out var lineParameter);
            if (lineParameter < 0)
                nearestPointOnLine = closestPoint;
            else if (lineParameter > 1) nearestPointOnLine = secondClosestPoint;
        ...
        public (int lowerIndex, int higherIndex) GetIndices() {
            ...
            Debug.Assert(indicesOneApart.Length == 1,
                $"Expected exactly one pair of indices one apart, but found {indicesOneApart.Length} on {this}.");
        ...
        /// <param name="existingSpline">
        ///     The spline that should be used for the segment, or null if a new spline will be generated
        ///     and set afterwards
        /// </param>
        ...
        internal static bool GenerateStreetSegment(SplineStreetNode from, SplineStreetNode to,
            [CanBeNull] Spline existingSpline, out SplineStreetSegment newSegment) {
            ...
            if (existingSpline != null) newSegment.Spline = existingSpline;
