...
namespace Graph {
    public class FreeFormStreetGraphTest : MonoBehaviour {
        ...
        [SerializeField] private bool drawLabelsEdges;
        [SerializeField] private bool drawLabelsNodes;
        [SerializeField] private bool drawMouseLabel = true;
        [SerializeField] private float mouseNodeDistanceThreshold = 0.3f;
        [SerializeField] private bool allowMouseCreation;
        ...
        private void OnGUI() {
            ...
            if (GUILayout.Button($"Draw grid: ({drawGrid})")) drawGrid = !drawGrid;
            if (GUILayout.Button("Graph color: Recalculate colors")) GraphColoring();
            GUILayout.Label("Labels:");
            GUILayout.BeginHorizontal();
            if (GUILayout.Button($"Nodes ({drawLabelsNodes})")) drawLabelsNodes = !drawLabelsNodes;
            if (GUILayout.Button($"Edges ({drawLabelsEdges})")) drawLabelsEdges = !drawLabelsEdges;
            GUILayout.EndHorizontal();
            if (GUILayout.Button($"Mouse Label ({drawMouseLabel})")) drawMouseLabel = !drawMouseLabel;
        ...
        private void OnDrawGizmos() {
            ...
            try {
                ...
                if (drawGrid) {
                    ...
                    for (var i = 0; i < world.Width; i++) Gizmos.DrawRay(new Vector3(i, 0), Vector3.up * world.Height);
                ...
                if (drawLabelsEdges)
                    ...
                    foreach (var edge in edges)
                        Handles.Label((edge.PositionNodeA + edge.PositionNodeB) / 2, edge.DebugString());
        ...
        private void GraphColoring() {
            ...
            var colors = new List<Color> {
                Color.red, Color.blue, Color.yellow, Color.green, Color.cyan, Color.magenta, Color.grey, Color.black
            };
            ...
            while (queue.Count != 0) {
                ...
                foreach (var edge in current.Edges) {
                    ...
                    if (!visited.Contains(nextNode)) queue.Enqueue(nextNode);
                }
            }
        }
